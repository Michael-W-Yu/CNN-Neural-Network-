...
import layer_copy as layer
import optimizer
import filter_ops
import accelerated_layer as accel
...
class Network():
    ...
    ConvNet4 network.
    '''
    def __init__(self, reg=0, verbose=True, early_stop = False):
        ...
        for i in range(n_iter):
            ...
            if self.early_stop == True:
                if (len(self.loss_history )>3):
                    if (abs(self.loss_history[-1] - self.loss_history[-2]) <0.00001 ):
...
class ConvNet4(Network):
    ...
    def __init__(self, input_shape=(3, 32, 32), n_kers=(32,), ker_sz=(7,), dense_interior_units=(100,),
                 pooling_sizes=(2,), pooling_strides=(2,), n_classes=10, wt_scale=1e-3, reg=0, verbose=True, early_stop = False):
        '''
        Parameters:
        -----------
        ...
        '''
        super().__init__(reg, verbose, early_stop)
...
class ConvNet4Accel(Network):
    ...
    def __init__(self, input_shape=(3, 32, 32), n_kers=(32,), ker_sz=(7,), dense_interior_units=(100,),
        ...
        # 1) Input convolutional layer
        self.layers.append( accel.Conv2DAccel(0, 'conv2D', n_kers[0], ker_sz[0], n_chans=n_chans,wt_scale = wt_scale, activation='sigmoid', reg = reg))
        # 2) 2x2 max pooling layer
