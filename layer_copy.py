...
class Layer():
    ...
    # Extension
    def sigmoid(self):
        '''sigmoid activation function. See notebook for a refresher on the
        mathematical equation.
        ...
        No return
        '''
        self.net_act = 1/(1+np.exp(-self.net_in))
...
#         print('d_net_in', d_net_in)
    ...
    def backward_netAct_to_netIn(self, d_upstream, y):
        ...
        if self.activation == 'relu':
            ...
            d_net_in = d_upstream.copy()#*np.ones(d_upstream.shape)
            # TODO: compute correct gradient here
        elif self.activation == 'softmax':
            d_net_act = -1/(len(self.net_act) * self.net_act)
            # TODO: compute correct gradient here
            d_net_in = d_upstream*self.net_act*(self.one_hot(y,self.net_act.shape[1] ) - self.net_act)
        elif self.activation == 'sigmoid':
            d_net_act = self.net_act(1-self.net_act)
